...
######################################################################
# Task-1b: Total time for writing all operators, parallel (py) native 
######################################################################

# f_name = "./benchmark-points/all_native_par.yaml" 
# t_i = perf_counter()
# keys = [[],wc_names]
# write_to_yaml(f_name,models[0],param_dicts[0],keys,"threadpool",**scale) 
# t_f = perf_counter()
# print(f"File written in native format (using parallel py process) in {t_f - t_i:.2f}s.")

########################################################################
# Task-1c: Total time for writing all operators, parallel (C++) native 
########################################################################

# f_name = "./benchmark-points/all_native_par_c.yaml" 
# t_i = perf_counter()
# keys = [[],wc_names]
# write_to_yaml(f_name,models[0],param_dicts[0],keys,"omp",**scale) 
# t_f = perf_counter()
...
# write_to_yaml(f_name,models[0],param_dicts[0],keys,**scale) 
# t_f = perf_counter()
