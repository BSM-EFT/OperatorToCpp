...
wcxf_vs_yaml_all_seq(eft_info, models[0], param_dicts[0])

# Writing all operators, WCxf seq vs par
wcxf_seq_vs_par_all(eft_info, models[0])

# Writing all operators, native yaml, seq vs par
yaml_seq_vs_par_all(eft_info, models[0], param_dicts[0])

# Writing 59 operators of the SMEFT-d6 Warsaw basis, sequentially
wcxf_vs_yaml_SMEFTd6_59_seq(models[0], param_dicts[0])

# Writing 59 operators of the SMEFT-d6 Warsaw basis, in parallel
wcxf_vs_yaml_SMEFTd6_59_par(models[0], param_dicts[0])


# Single coefficient execution times
exec(models[0], "cH")
exec(models[0], "cuH_33")
exec(models[0], "cqd1_2213")
...
for i in range(5):
    ...
    write_to_wcxf(f_name,models[i],eft_info)
    t_f = perf_counter()
