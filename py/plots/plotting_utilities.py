...
def collect_vals(data, param_names, bmk_pts, wc_names):
    values = []
    for bp in bmk_pts:
        x, y = bp
        d1 = data[data[param_names[0]] == x]
        d2 = d1[d1[param_names[1]] == y]
...
def create_legend(param_names, bp):
    legend = r'%s $= {\rm %s\, TeV}$, %s = ${\rm %s\, TeV}$'%(label_dict[param_names[0]], bp[0], label_dict[param_names[1]], bp[1])
    return legend

# function for making bar plots

def plot_fn_bar(data, param_names, bmk_pts, wc_names, filename,**kwargs):
    ...
    vals = collect_vals(data, param_names, bmk_pts, wc_names)

    ax.bar(x_pos - bar_width, vals[2], width=bar_width, label=create_legend(param_names,bmk_pts[2]), color='#FFFFAA', edgecolor='black', alpha=0.85)
    ax.bar(x_pos, vals[1], width=bar_width, label=create_legend(param_names,bmk_pts[1]), color='tab:red', edgecolor='black', alpha=0.75)
    ax.bar(x_pos + bar_width, vals[0], width=bar_width, label=create_legend(param_names,bmk_pts[0]), color='#030764', edgecolor='black', alpha=0.85)
